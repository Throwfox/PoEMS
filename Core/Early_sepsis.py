...
class SepsisPolicyNetwork(nn.Module):

    def __init__(self, n_feature, hidden_size = Config.hidden_size,lock=False,hidden_output_size = 1, output_size = 1):
        super(SepsisPolicyNetwork, self).__init__()
        self.INPUT_SIZE = n_feature
        self.hidden_size = hidden_size
        ...
        #------lock the para before this line-------
        if lock:
    ...
    def applyLoss(self, y_hat, Y, gamma = 1, beta = 0.01):
        ...
        adjusted_reward_value=torch.sum(adjusted_reward, dim=1).mean()
